...
class Primitive2Macros
{
	macro public static function projectPointToLine( dstX, dstY, pX, pY, lposX, lposY, lvecX, lvecY )
	{
		return macro {
			...
			var dvx:Float = $pX - lx;
			var dvy:Float = $pY - lY;
			var m:Float = Vecop2Macros.dotProd(lvx, lvy, dvx, dvy) / Vecop2Macros.magnitude(lvx, lvy);
			$dstX = lx + m * lvx;
			$dstY = ly + m * lvy;
		}
	}
	...
	macro public static function tangentPointToCircle( dstX1, dstY1, dstX2, dstY2, dstCount, pX, pY, cposX, cposY, cRadius )
	{
		return macro {
			...
			var cvx:Float = cx - $pX;
			var cvy:Float = cy - $pY;
			var cvmag:Float = Vecop2Macros.magnitude(cvx, cvy);
			...
			if (cvmag < cr2) {
				...
				var dvx:Float = -m * cvx;
				var dvy:Float = -m * cvy;
				var dvmag:Float = Vecop2Macros.magnitude(dvx, dvy);
