...
class Vecop2_resTest
{
	...
	public function testPolar():Void
	{
		...
		for (item in items) {
			var dst:Vec2 = Vecop2.polar(item.radians, item.len);
	...
	public function testRotate():Void
	{
		...
		for (item in items) {
			var v:Vec2 = new Vec2(item.x, item.y);
			var dst:Vec2 = Vecop2.rotate(v, item.rad);
	...
	public function testRotateQuart():Void
	{
		...
		for (item in items) {
			var v:Vec2 = new Vec2(item.x, item.y);
			var dst:Vec2 = Vecop2.rotateQuart(v, item.times);
	...
	public function testMirrorBy():Void {
		...
		for (item in items) {
			var v:Vec2 = new Vec2(item.vx, item.vy);
			var dir:Vec2 = new Vec2(item.dirx, item.diry);
